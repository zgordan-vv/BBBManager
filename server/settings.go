...
func evaluateTomcat(params [][]string, tmpl map[string]Param) bool {
...
func setTomcatHandler(w http.ResponseWriter, r *http.Request) {
	...
	if !evaluateTomcat(params, tomcatTmpl) {out500(w); return}

	fmt.Println("stage 4")
	file := "/var/lib/tomcat7/webapps/bigbluebutton/WEB-INF/classes/bigbluebutton.properties"
	
	output, err := execPlain(params, file)
	fmt.Println("stage 5")
